    ...
    def check_status(self):
        ...
        if not latest_backup_log.ended_at:
            ...
            if async_result.status in [
                ...
                # If the task has started or is retrying then we should just
                # wait until it's finished to check everything else so wait
        ...
        document_s3_keys = set(
            Document.objects.filter(created_at__lte=latest_backup_log.ended_at, safe=True).values_list(
                "s3_key", flat=True
            )
        )
        backed_up_s3_keys = set(DocumentData.objects.values_list("s3_key", flat=True))
        not_backed_up = document_s3_keys - backed_up_s3_keys
        if not_backed_up:
            raise BackupDocumentDataHealthCheckException(f"{len(not_backed_up)} files missing from backup")
